...
class TourCommandControllerTest extends AcceptanceTest {
    ...
    void Should_DeleteTour() {
        // given
        String accessToken = USER_HAVE_SINGED_IN(AuthSteps.USERNAME, AuthSteps.EMAIL, AuthSteps.PASSWORD);
        ExtractableResponse<Response> createResponse = TOUR_CREATE_REQUEST(TourFixtures.TITLE, TourFixtures.SUMMARY, TourFixtures.TYPE, TourFixtures.DEPARTURE_POINT, TourFixtures.ARRIVAL_POINT, TourFixtures.START_AT, TourFixtures.END_AT, accessToken);
        long tourId = createResponse.jsonPath().getLong("tourId");

        // when
        ExtractableResponse<Response> response = TOUR_DELETE_REQUEST(tourId, accessToken);

        // then
        assertThat(response.statusCode()).isEqualTo(HttpStatus.NO_CONTENT.value());
        assertThat(TOUR_LIST_REQUEST().jsonPath().getList("tours")).isEmpty();
    }
}